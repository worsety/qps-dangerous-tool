...
namespace const_obfuscate {
    ...
    template<size_t N>
    constexpr auto obfuscate(const char (&cleartext)[N], const unsigned seed) {
        struct { char str[N]; } ret{};
        for (unsigned i = 0, state = seed; i < N; i++, state = next_rand(state))
            ret.str[i] = cleartext[i] ^ state;
        return ret;
    }
    ...
    template<size_t N>
    auto deobfuscate(const volatile char (&cleartext)[N], const unsigned seed) {
        struct { char str[N]; } ret{};
        for (unsigned i = 0, state = seed; i < N; i++, state = next_rand(state))
            ret.str[i] = cleartext[i] ^ state;
        return ret;
...
#define obf_str(STRING) ([&] { \
    constexpr auto _ciphertext{const_obfuscate::obfuscate(STRING, __LINE__)}; \
    const volatile auto _ciphertextCopy{_ciphertext}; \
    return const_obfuscate::deobfuscate(_ciphertextCopy.str, __LINE__); \
}().str)
...
#define OBF_IMPORT(module, func) do { \
    ...
    OBF_FUNC(func) = reinterpret_cast<decltype(OBF_FUNC(func))>(GetProcAddress(hModule, obf_str(#func))); \
    CloseHandle(hModule); \
} while(0)
